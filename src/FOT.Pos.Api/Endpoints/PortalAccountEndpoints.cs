...
public static class PortalAccountEndpoints
{
    public static void MapPortalAccountEndpoints(this RouteGroupBuilder api)
    {
        ...
        {
            if (!IsAdmin(http)) return Results.Forbid();
            var ok = await repo.SetSellerActiveAsync(id, active, default);
            return ok ? Results.Ok() : Results.NotFound(new { error = "لا يوجد حساب لهذا البائع" });
        });
        ...
        {
            if (!IsAdmin(http)) return Results.Forbid();
            var ok = await repo.SetManagerActiveAsync(id, active, default);
