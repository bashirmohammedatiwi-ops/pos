...
#define MyAppDisplayName "FOT POS إدارة عن بُعد"
#define MyAppUninstallId "{8C3E9A21-7F04-4B55-9D2E-A1B2C3D4E5F2}"
#define SourceDir "..\publish\desktop\FOT-POS-Remote"
#include "FOT-POS-Upgrade-Messages.issinc"

[Setup]
AppId={{8C3E9A21-7F04-4B55-9D2E-A1B2C3D4E5F2}
AppName={#MyAppDisplayName}
AppVersion={#MyAppVersion}
AppVerName={#MyAppDisplayName} {#MyAppVersion}
AppPublisher={#MyAppPublisher}
DefaultDirName={autopf}\FOT POS\Remote
DefaultGroupName=FOT POS
DisableProgramGroupPage=yes
DisableDirPage=auto
OutputDir=..\publish\installers
OutputBaseFilename=FOT-POS-Remote-Setup
SetupIconFile=..\assets\branding\fot-pos-admin.ico
UninstallDisplayIcon={app}\FOT.Pos.Admin.Remote.exe
...
VersionInfoProductName=FOT POS Remote Admin
...
[Icons]
Name: "{group}\إدارة عن بُعد"; Filename: "{app}\FOT.Pos.Admin.Remote.exe"; WorkingDir: "{app}"
Name: "{autodesktop}\FOT POS إدارة عن بُعد"; Filename: "{app}\FOT.Pos.Admin.Remote.exe"; WorkingDir: "{app}"; Tasks: desktopicon

[Run]
Filename: "{app}\FOT.Pos.Admin.Remote.exe"; Description: "تشغيل لوحة التحكم عن بُعد"; Flags: nowait postinstall skipifsilent
...
var Path: String; Content: AnsiString; Marker, Rest: String; P: Integer;
begin
  Result := '';
  ...
  begin
    ...
    if P > 0 then begin Rest := Copy(Content, P + Length(Marker), MaxInt); P := Pos('"', Rest);
      if P > 0 then Result := Copy(Rest, 1, P - 1); end;
  end;
end;
...
begin
  ...
  if (Pos('http://', LowerCase(Url)) <> 1) and (Pos('https://', LowerCase(Url)) <> 1) then Url := 'http://' + Url;
  while (Length(Url) > 0) and (Url[Length(Url)] = '/') do Url := Copy(Url, 1, Length(Url) - 1);
...
begin
  ...
  begin
    if MsgBox(FmtMessage(CustomMessage('FOTSameVersionPrompt'), [AppDisplayName(), PrevAppVersion]),
      mbConfirmation, MB_YESNO or MB_DEFBUTTON2) = IDNO then begin Result := False; Exit; end;
  end;
  Result := True;
end;
...
begin
  ...
  if SavedUrl <> '' then ServerPage.Values[0] := SavedUrl else ServerPage.Values[0] := 'http://192.168.75.1:5000';
  ApplyWizardTexts();
end;
...
begin
  ...
  Json := '{' + #13#10 + '  "DeploymentRole": "Remote",' + #13#10 +
    '  "ApiBaseUrl": "' + Url + '",' + #13#10 + '  "ApiPort": 5000' + #13#10 + '}';
...
begin
  Exec('powershell.exe', '-NoProfile -Command "Get-Process FOT.Pos.Admin* -ErrorAction SilentlyContinue | Stop-Process -Force -ErrorAction SilentlyContinue"', '', SW_HIDE, ewWaitUntilTerminated, ResultCode);
  Result := '';
end;
